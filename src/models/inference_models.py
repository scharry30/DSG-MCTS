...
def get_llama3_response(query, model, tokenizer, max_length=2048, truncation=True, max_new_tokens=1024, temperature=0.7, do_sample=False):
    cnt = 2
    all_response = ''
...
class Llama_PRM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        true_token_id = self.base_model.config.tokenizer.convert_tokens_to_ids("true")
        output = probs[:, -1, true_token_id]
        return output
