...
def randomPolicy(node: treeNode, mcts_task):
    ...
    reflection = mcts_task.get_reflection(strs, cur_step) if mcts_task.use_reflection == 'common' else mcts_task.get_simple_reflection(strs, cur_step)
    ...
    for i in range(mcts_task.roll_forward_steps):
        ...
        if value > max_V:
            max_V = value
        cur_ref = mcts_task.get_reflection(strs, cur_step) if mcts_task.use_reflection == 'common' else mcts_task.get_simple_reflection(strs, cur_step)
        if cur_ref == '<end>':
            break
    return max_V
...
def MCTS_search(mcts_task):
    ...
    if mcts_task.limit_type == 'time':
        ...
        while time.time() < timeLimit:
            print(f'<Start new search cycle, elapsed time:{time.time() - time_start}>\n')
        ...
        for i in range(mcts_task.iteration_limit):
            print(f'<Start new search cycle, completed cycles: {i}>\n')
            flag, node, root = executeRound(root, mcts_task)
            if flag:
                print('Solution found!\n')
...
def executeRound(root, mcts_task):
    """Execute a selection-expansion-simulation-backpropagation cycle."""
    print('-' * 40)
...
def isTerminal(node, mcts_task):
    return node.V >= mcts_task.end_gate if mcts_task.reward_model_type == 'vm' else False
...
def expand(node: treeNode, mcts_task):
    if not node.reflection:
        reflection = mcts_task.get_reflection(node.y, node.depth + 1) if mcts_task.use_reflection == 'common' else mcts_task.get_simple_reflection(node.y, node.depth + 1)
        node.update_reflection(reflection)
    ...
    for action in actions:
        if action not in node.children:
            ...
            if mcts_task.sample_value == 'full':
                child.update_reflection(mcts_task.get_reflection(child.y, child.depth + 1) if mcts_task.use_reflection == 'common' else mcts_task.get_simple_reflection(child.y, child.depth + 1))
